...
const onConnect = ws => {
  ...
  ws.onmessage(MESSAGE.client.requestMap, sendMap.bind(ws))

}
const sendMap = async function() {
  let planets = await DB.getAllPlanets()
  let response = { planets }
  this.sendMessage(MESSAGE.server.sendMap, response)
}
